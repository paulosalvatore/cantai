...
Dependency edges: 6 → {7, 9, 11, 12}; 8 → 9; {3-lib, 7, 9} → 10; 18 → (7's small `/tv` pause-state read lands post-18).
...
5. **Shared-file protocol:** exactly one sanctioned overlap in the batch — #12's one-line `track()` calls in routes owned by #6/#7/#8, additive-only, lands last.
